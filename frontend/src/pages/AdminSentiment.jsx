...
function sentimentCategoryTip(category) {
  const { reassured_min, equanimity_min } = SENTIMENT_THRESHOLDS
  if (category === 'Clinically Reassured') {
    return `Clinically Reassured: Sentiment Score ≥ ${reassured_min}%. High overall alignment: positive feedback, low frustration signals, and clinically stable conversation patterns.`
  }
  if (category === 'Clinical Equanimity') {
    return `Clinical Equanimity: Sentiment Score ${equanimity_min}%–${reassured_min - 0.1}%. Moderate alignment: generally stable, with some friction or mixed feedback that may benefit from follow-up.`
  }
  if (category === 'Clinical Distress') {
    return `Clinical Distress: Sentiment Score < ${equanimity_min}%. Low alignment: higher frustration and/or negative feedback trends. May require closer follow-up or escalation.`
  }
  return ''
}
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-6">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <h2 className="font-disp font-bold text-xl text-[var(--text-main)]">
            Patient Sentiment & Telemetry Scorecard
          </h2>
          <ul className="text-sm text-[var(--text-dim)] mt-1 max-w-5xl leading-relaxed list-disc pl-5 space-y-1">
            <li>Sentiment Scorecard is computed per patient using a weighted formula: Sentiment Score = (Feedback Score% x 0.40) + (Frustration Inverse% x 0.30) + (CQS% x 0.30).</li>
            <li>Feedback Score% comes from average rating (1-5 scaled to 0-100), Frustration Inverse% is 100 - average conversation frustration, and CQS% is the patient quality score from recent sessions.</li>
            <li>Clinical categories use fixed thresholds: Clinically Reassured &gt;= 75%, Clinical Equanimity 50-74.9%, and Clinical Distress &lt; 50%.</li>
            <li>Intervention is automatically flagged when score is below 50%, average frustration is high (&gt;55), feedback is low (&lt;=2.5), or conversation escalation is detected.</li>
          </ul>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <MetricCard
          ...
          tip={sentimentCategoryTip('Clinically Reassured')}
        />
        <MetricCard
          label="Clinical Equanimity"
          value={overallStats.equanimity_count}
          color="#818CF8"
          icon={<Compass size={14} />}
          sub="Moderate therapeutic alignment"
          tip={sentimentCategoryTip('Clinical Equanimity')}
        />
        <MetricCard
          ...
          tip={sentimentCategoryTip('Clinical Distress')}
        />
      </div>
              ...
              {filteredPatients.map((p, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Category */}
                    <td className="px-4 py-3">
                      <span title={sentimentCategoryTip(p.sentiment_category)}>
