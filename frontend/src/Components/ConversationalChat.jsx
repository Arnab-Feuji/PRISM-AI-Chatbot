...
// Answer body colours aligned with badge chips (readable on dark patient chat)
const ANSWER_THEME = {
  progressive: { text: "#93C5FD", bg: "rgba(37, 99, 235, 0.12)",  border: "rgba(37, 99, 235, 0.45)" },
  narrative:   { text: "#C4B5FD", bg: "rgba(124, 58, 237, 0.12)", border: "rgba(124, 58, 237, 0.45)" },
  quick_tips:  { text: "#6EE7B7", bg: "rgba(5, 150, 105, 0.12)",  border: "rgba(5, 150, 105, 0.45)" },
  real_talk:   { text: "#FCA5A5", bg: "rgba(220, 38, 38, 0.12)",  border: "rgba(220, 38, 38, 0.45)" },
  comparison:  { text: "#FCD34D", bg: "rgba(217, 119, 6, 0.12)",  border: "rgba(217, 119, 6, 0.45)" },
  staged:      { text: "#67E8F9", bg: "rgba(8, 145, 178, 0.12)",  border: "rgba(8, 145, 178, 0.45)" },
  myth_bust:   { text: "#D8B4FE", bg: "rgba(147, 51, 234, 0.12)", border: "rgba(147, 51, 234, 0.45)" },
  bridge:      { text: "#CBD5E1", bg: "rgba(71, 85, 105, 0.12)",  border: "rgba(71, 85, 105, 0.45)" },
  specialist:  { text: "#FCD34D", bg: "rgba(245, 200, 66, 0.12)", border: "rgba(245, 200, 66, 0.5)" },
  human:       { text: "#FCA5A5", bg: "rgba(185, 28, 28, 0.12)",  border: "rgba(185, 28, 28, 0.5)" },
  personalised:{ text: "#6EE7B7", bg: "rgba(5, 150, 105, 0.12)",  border: "rgba(52, 211, 153, 0.5)" },
};

/** Pick answer text theme when multiple badges show: Specialist > Personalised > format. */
function getAnswerTheme(m) {
  if (m.routeDecision === "human") return ANSWER_THEME.human;
  if (m.routeDecision === "specialist") return ANSWER_THEME.specialist;
  if (m.contextCollected && m.slotsCount > 0) return ANSWER_THEME.personalised;
  if (m.responseFormat && ANSWER_THEME[m.responseFormat]) return ANSWER_THEME[m.responseFormat];
  return null;
}
        ...
        if (line.startsWith("- ") || line.startsWith("• "))
          return (
            <div key={i} style={{ display: "flex", gap: 7, paddingLeft: 8, marginBottom: 3, alignItems: "flex-start" }}>
              <span style={{ color: "currentColor", opacity: 0.65, flexShrink: 0, marginTop: 4, fontSize: 7 }}>●</span>
...
export function ConversationalMessage({
  ...
  const answerTheme = getAnswerTheme(m);
    ...
    <div style={{ marginBottom: 4, animation: "slideIn .2s ease" }}>
      <div style={{ display: "flex", gap: 9 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* Answer bubble — text colour matches active response badges */}
          <div style={{
            background:   answerTheme ? answerTheme.bg : "var(--color-background-primary)",
            border:       answerTheme
              ? `1px solid ${answerTheme.border}`
              : "0.5px solid var(--color-border-tertiary)",
            borderLeft:   answerTheme ? `3px solid ${answerTheme.border}` : undefined,
            borderRadius: "2px 12px 12px 12px",
            padding:      "11px 15px",
            boxShadow:    answerTheme ? "none" : "0 1px 3px rgba(0,0,0,.04)",
            color:        answerTheme ? answerTheme.text : "var(--color-text-primary)",
          }}>
            <MD text={m.content} />
