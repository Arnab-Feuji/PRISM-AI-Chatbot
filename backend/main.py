...
# ── Uploaded-document context limits (prevent token overflow & meta bloat) ─────
_DOC_CONTEXT_TEXT_MAX     = 12_000
_DOC_CONTEXT_ANALYSIS_MAX = 6_000
_DOC_META_STORE_TEXT_MAX  = 24_000
_MAX_DOCS_IN_PROMPT       = 3
...
async def _fetch_cross_conversation_uploads(db, user_id: str, uploaded_docs: list) -> list:
    """Append recent ImageUpload rows not already present in uploaded_docs."""
    ...
    try:
        ...
        for upload in db_res.scalars().all():
    ...
    return uploaded_docs[-_MAX_DOCS_IN_PROMPT:]


def _build_document_context_addendum(uploaded_docs: list) -> str:
    ...
    for i, doc in enumerate(uploaded_docs, start=1):
        ...
        doc_text = _truncate_text(doc.get("text", ""), _DOC_CONTEXT_TEXT_MAX)
        patient_q = doc.get("patient_query", "")
        prev_resp = _truncate_text(doc.get("analysis_response", ""), _DOC_CONTEXT_ANALYSIS_MAX)
        part = (
    ...
    return (
        ...
        f"3. Use your clinical expertise to bridge any gap between the document findings and the active agent.\n"
...
async def chat(
    ...
    has_uploaded_docs = bool(meta.get("uploaded_documents"))
    ...
    # Document conversations: answer directly instead of clarifying loops
    if not force_immediate and (has_uploaded_docs or last_analysis):
        force_immediate = True
    ...
    # Inject multi-document clinical context (preserve conversation_state set above)
    uploaded_docs = list(meta.get("uploaded_documents", []))
    uploaded_docs = await _fetch_cross_conversation_uploads(db, user_id, uploaded_docs)
    if uploaded_docs:
        context_addendum += _build_document_context_addendum(uploaded_docs)
...
async def chat_image(
    ...
    try:
        ...
        raw_text = extracted_text if analysis_type == "document" else validation.get("description", "")
        uploaded_docs.append({
            "filename": filename,
            "label": doc_label,
            "type": analysis_type,
            "text": _truncate_text(raw_text, _DOC_META_STORE_TEXT_MAX),
            "patient_query": query,
            "analysis_response": analysis["response"],
            "severity": analysis.get("severity", "Normal"),
            "timestamp": datetime.utcnow().isoformat()
        })
        meta["uploaded_documents"] = uploaded_docs[-10:]
...
async def admin_overview(current: dict = Depends(require_admin), db: AsyncSession = Depends(get_db)):
    # Exclude synthetic test users from dashboard counts.
    res_users = await db.execute(select(User).where(User.role == "patient"))
    patients = res_users.scalars().all()
    users_count = sum(
        1 for u in patients
        if not (
            (u.email or "").lower().startswith("test_")
            or ((u.name or "").strip().lower() == "test user")
        )
    )
...
async def admin_vector_store(
    ...
    from backend.core.rag.pipeline import get_chroma_mode, get_chroma

    pipeline = get_rag_pipeline()
    live_counts = pipeline.store.all_collection_counts()

    # DB totals (indexed_documents) — fallback when Chroma is out of sync
    res = await db.execute(
        select(
            IndexedDocument.collection_name,
            func.coalesce(func.sum(IndexedDocument.chunk_count), 0),
            func.count(IndexedDocument.id),
        )
        .where(IndexedDocument.is_active == True)
        .group_by(IndexedDocument.collection_name)
    )
    db_rows = {row[0]: {"chunks": int(row[1] or 0), "docs": int(row[2] or 0)} for row in res.all()}

    stats = []
    for col in ALL_COLLECTIONS:
        chroma_n = int(live_counts.get(col, 0))
        db_info = db_rows.get(col, {"chunks": 0, "docs": 0})
        db_n = db_info["chunks"]
        # Display live Chroma count; use DB sum only if Chroma is empty but DB has indexed chunks
        display_n = chroma_n if chroma_n > 0 else db_n
        stats.append({
            "collection": col,
            "document_count": display_n,
            "chroma_count": chroma_n,
            "db_chunk_count": db_n,
            "indexed_documents": db_info["docs"],
            "in_sync": chroma_n == db_n or (chroma_n > 0 and chroma_n >= db_n),
        })

    # Include any Chroma collections not in ALL_COLLECTIONS (legacy / manual)
    for col_name, chroma_n in live_counts.items():
        if col_name not in ALL_COLLECTIONS and chroma_n > 0:
            stats.append({
                "collection": col_name,
                "document_count": chroma_n,
                "chroma_count": chroma_n,
                "db_chunk_count": 0,
                "indexed_documents": 0,
                "in_sync": True,
    ...
    return {
        "collections": stats,
        "total_chunks": sum(s["document_count"] for s in stats),
        "total_chroma_chunks": sum(s["chroma_count"] for s in stats),
        "chroma_mode": get_chroma_mode(),
        "chroma_path": get_settings().chroma_persist_path,
