...
class PRISMRetrievalPipeline:
    ...
    async def retrieve(
        ...
        # Stage 1: ChromaDB bi-encoder retrieval (top-10) — must use BGE embeddings
        from backend.core.rag.pipeline import query_collection_with_embedder
        raw_results = query_collection_with_embedder(
            collection,
            query,
