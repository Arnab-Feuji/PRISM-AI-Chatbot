...
def get_chroma() -> chromadb.ClientAPI:
    ...
    print(f"[CHROMA] Initializing local persistent storage at {persist_path}")
...
def get_chroma_mode() -> str:
    """Return 'remote' or 'local' after get_chroma() has initialized the client."""
    get_chroma()
    return _chroma_mode or "local"


def query_collection_with_embedder(
    collection,
    query: str,
    n_results: int = 10,
    ...
    """
    Query a Chroma collection using the same BGE embedder used at ingest time.
    Avoids query_texts, which uses Chroma's default 384-dim model and breaks
    collections indexed with 768-dim BGE vectors.
    """
...
class PRISMVectorStore:
    ...
    def all_collection_counts(self) -> Dict[str, int]:
        """Live ChromaDB counts keyed by collection name (fresh read, no stale handles)."""
        counts: Dict[str, int] = {}
        self._cols.clear()
        for col in self.client.list_collections():
            try:
                counts[col.name] = self.client.get_collection(col.name).count()
            except Exception:
                counts[col.name] = 0
        return counts
